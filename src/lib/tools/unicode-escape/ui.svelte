<script lang="ts">
	...
	import { unicodeEscape, run, type UnicodeEscapeInput } from './index';

	const shareKeys = unicodeEscape.share!.params;
	const DEFAULT_TEXT = 'café ☕';
	const DEFAULT_MODE: UnicodeEscapeInput['mode'] = 'escape';

	function fromUrl() {
		const sp = urlSearchParams();
		const modeRaw = readShareParam(sp, 'mode');
		const mode: UnicodeEscapeInput['mode'] =
			modeRaw === 'unescape' || modeRaw === 'escape' ? modeRaw : DEFAULT_MODE;
		return {
	...
	let mode = $state<UnicodeEscapeInput['mode']>(initial.mode);
...
<div class="flex max-w-2xl flex-col gap-4">
	...
	<Field id="unicode-input" label="Input">
		<Textarea id="unicode-input" bind:value={text} rows={6} class="font-mono text-sm" />
	</Field>
	...
	<Field id="unicode-output" label="Output">
		<Textarea
			id="unicode-output"
			value={output.result}
			rows={6}
